...
# Phoneme category mapping (keeping the same as before)
...
def extract_noise_type(filename):
    """
    Extract the noise type from the ANSD neurogram filename.
    ...
    """
    # Check for default noise first
    if 'default' in filename:
        return 'default'
    
    # Try to match SNR pattern specific to ANSD neurograms
    snr_match = re.search(r'ansd_snr([+-]?\d+\.\d+)', filename)
    if snr_match:
    ...
    for dr_folder in sorted(os.listdir(base_path)):
        ...
        for speaker_folder in sorted(os.listdir(dr_path)):
            ...
            for mat_file in sorted(os.listdir(speaker_path)):
                ...
                if not mat_file.endswith('.mat') or '_with_noise_ansd_' not in mat_file:
                    continue
                ...
                try:
                    ...
                    if 'r_mean_downsampled' in data:
                        ...
                        # Format: SA1_aa_19962_21514_neurogram_with_noise_ansd_default.mat
                        ...
                        if len(parts) >= 3:
                            ...
                            neurogram_row = {
                                ...
                                'SelectedFrequencies': selected_frequencies,
                                'FilePath': file_path,
                                'PerturbationType': data.get('perturbation_type', ['Unknown'])[0]
                            }
...
def main():
    # File paths
    neurogram_base_path = '/Users/mcampi/Desktop/To_Marta/ANSD_Study/code/code_all/SELECT_Neurograms_ANSD_WithNoise/TEST/'
    formants_base_path = '/Users/mcampi/Desktop/To_Marta/ANSD_Study/code/code_all/SELECT_Formants/Phonemes/TEST/'
    output_dir = '/Users/mcampi/Desktop/To_Marta/ANSD_Study/code/code_all/SELECT_F_vs_Neuro_ANSD_Noise_Results/'
    ...
    # Check distribution of noise types and perturbation types
    ...
    if 'PerturbationType' in df_neurograms.columns:
        perturb_counts = df_neurograms['PerturbationType'].value_counts()
        print("\nPerturbation type distribution:")
        print(perturb_counts)
    ...
    merged_df = pd.concat([merged_df, noisy_means], axis=1)
    ...
    # Save merged data
    merged_df.to_pickle(os.path.join(output_dir, 'merged_data_ansd_noise.pkl'))
    
    # Process each noise type separately 
    all_results = []
    
    print("\nProcessing neurograms")
    ...
    if not distance_results.empty:
        # Save results
        distance_results.to_pickle(os.path.join(output_dir, 'formant_cf_distances_all_ansd.pkl'))
    ...
    for noise_type in NOISE_TYPES:
        ...
        # Compute distances
        distance_results = compute_formant_cf_distances(merged_df[merged_df['NoiseType'] == noise_type])
        
        if not distance_results.empty:
            # Save results
            distance_results.to_pickle(os.path.join(output_dir, f'formant_cf_distances_{noise_type}_ansd.pkl'))
            
            # Create visualizations
            create_visualizations(distance_results, output_dir, noise_type)
    ...
    if all_results:
        ...
        combined_results.to_pickle(os.path.join(output_dir, 'formant_cf_distances_all_combined_ansd.pkl'))
        
        # Create comparison visualizations
        compare_noise_types(combined_results, output_dir)
        
        # Compare clean vs noisy (skipped as we don't have clean neurograms)

...
function process_directory(input_base_dir, output_base_dir, fsmod, center_frequencies)
    ...
    for dr_idx = 1:length(dr_dirs)
        ...
        for spk_idx = 1:length(speaker_dirs)
            ...
            for wav_idx = 1:length(wav_files)
                ...
                % Generate neurogram
                [r_mean, ~, ~, ~, ~, ~, output] = zilany2014(audio, fs, center_frequencies, ...
                ...
                % Save neurogram
                save(output_file, 'r_mean', 'center_frequencies');
...
process_directory(train_input_dir, train_output_dir, fsmod, center_frequencies);
...
process_directory(test_input_dir, test_output_dir, fsmod, center_frequencies);

...
from balance_indices import generate_balanced_indices  # Changed to simpler function
...
# Generate balanced position indices for training using the simpler function
print("\nGenerating balanced position indices...")

try:
    # This function is simpler and more reliable
...
try:
    ...
    # Adjust batch size for multi-GPU - INCREASED
    try:
        # Use larger batch size per replica
        BATCH_SIZE_PER_REPLICA = 256  # Increased from 128
    ...
    #-------------------------------------------------------------------------------
    # NON-CAUSAL PART - UPDATED FOR NEW srA2.py
    #-------------------------------------------------------------------------------
    
    # Adjust batch size for non-causal part - INCREASED
    try:
        BATCH_SIZE_PER_REPLICA = 512  # Increased from 128 to 512
        GLOBAL_BATCH_SIZE = BATCH_SIZE_PER_REPLICA * strategy.num_replicas_in_sync
    except:
    ...
    with strategy.scope():
        trainNonCausalNN(
            ...
            batch_size=GLOBAL_BATCH_SIZE,
            reduce_factor=10,
            # Pass the validation positions path to use pre-split validation data
            validation_positions_path=validation_positions_path,
            # NEW: Pass balanced positions to use the same balanced data as srA1
            balanced_positions_path=balanced_positions_path
        )
    ...
    with strategy.scope():
        trainNonCausalNN(
            ...
            epochs_total=10,
            batch_size=GLOBAL_BATCH_SIZE,
            reduce_factor=1,  # No reduction for final training
    ...
    # Final predictions - Updated to handle the new predictions format

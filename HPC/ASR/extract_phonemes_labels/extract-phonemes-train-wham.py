...
def extract_phoneme_labels(data_dir, output_dir, dataset_type="train"):
    """
    ...
    # Regex pattern to match files like: SA1_aa_14800_16835_neurogram_with_noise.mat
    pattern = r'([A-Z]+\d+)_([a-z\-#]+)_(\d+)_(\d+)_neurogram_with_noise\.mat'
    ...
    for root, dirs, files in os.walk(data_dir):
        for file in files:
            ...
            # Process only neurogram files with the specific pattern
            if not file.endswith('_neurogram_with_noise.mat'):
                continue
    ...
    if not all_phonemes:
        ...
        print("Make sure files follow the pattern: SA1_aa_14800_16835_neurogram_with_noise.mat")
    ...
    # Create output filenames to match the pattern of the test set files
    phoneme_file = f"Phonemes39consecutive_true_{dataset_type}_healthy_noise.npy"
    position_file = f"Phonemes39_position_index_true_{dataset_type}_healthy_noise.npy"
    mapping_file = f"phoneme_mapping_{dataset_type}_healthy_noise.txt"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description='Extract phoneme labels from WHAM TIMIT neurogram filenames.')
    ...
    # Process the data
    extract_phoneme_labels(args.data_dir, args.output_dir, args.dataset)
